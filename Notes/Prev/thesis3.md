...
As online communication gained more popularity, using instant messaging applications has become a standard in our quotidian lives. Therefore, the need for assurance that there is no third party spying on our conversations, either the government, the service provider or an attacker with malicious intent, grew even more, especially in states where free speech is threatened.
...
In case the service providers, compliance with regulations regarding data privacy and security is easier [?]. 

But the cost of benefits is paid with a number of limitations. Some of these are collection of user metadata, which can be used for advertising[?]. This issue was tackled by Signal with its Sealed Sender feature[?]. 

Also, backdoors are heavily requested by governments in order to protect the citizens and fight crimes[?] and, therefore, spam and abuse becomes harder to control on end-to-end encrypted platforms. More will be discussed in [Section 2](#2), along with a brief presentation of the cryptographic concepts used throughout the thesis. 

Messaging apps using end-to-end encryption have been around since 2012, with iPhone's native messaging app iMessage[1] and then Signal[4], previously known as TextSecure, developed in 2013. This practice became more popular in 2016, when Whatsapp announced the introduction of end-to-end encryption by default in the application[2] and Facebook Messenger added the "secret chats" feature with similar properties[3]. 

The purpose of this thesis is to create an overview of the most used end-to-end encryption protocols in popular instant messaging apps and their mechanisms. The topics that will be discussed are their current implementations and innovations in the field, as well as past security issues. The protocols analyzed are Signal, MTProto, Signcryption, Letter Sealing and Threema, in [Section 3](#3-existing-technologies). 

## The application
The application is a web-based instant messaging application which provides end-to-end encryption, by default, for both private and group chats. The users register with an email and can send messages and attachments. 
